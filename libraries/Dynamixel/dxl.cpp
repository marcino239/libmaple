...
void dx_debug_helper( char *name, unsigned int val, unsigned int base ) {
	SerialUSB.print( name );
	SerialUSB.print( ": " );
	SerialUSB.println( val, base );

}

void dxl_debug_packet( struct dxl_packet *packet ) {
	SerialUSB.print( "id" );
	SerialUSB.print( ": " );
	SerialUSB.println( packet->id, HEX );
  
	SerialUSB.print( "instr" );
	SerialUSB.print( ": " );
	SerialUSB.println( packet->instruction, HEX );

	SerialUSB.print( "parameter_nb" );
	SerialUSB.print( ": " );
	SerialUSB.println( packet->parameter_nb, HEX );
	
	if( packet->parameter_nb > 0 ) {
	  SerialUSB.print( "params: " );
	  for( int i=0; i<packet->parameter_nb; i++ ) {
		  SerialUSB.print( packet->parameters[i], HEX );
		  SerialUSB.print( ", " );
	  }
	  SerialUSB.println();
	}
...
void dxl_init(int baudrate)
{
    ...
    dxl_timeout = 100000000/baudrate;
...
//    SerialUSB.println( "dxl_packet timeout" );
    return NULL;
}
...
void dxl_set_position(ui8 id, uint16 position) {
  if (id < DXL_MAX_ID && id != 0) {
    dxl_write_word(id, DXL_GOAL_POSITION, position);
  }
}
...
void dxl_reset( ui8 id )
{
    struct dxl_packet request;
    request.id = id;
    request.instruction = DXL_CMD_RESET;
    request.parameter_nb = 0;
